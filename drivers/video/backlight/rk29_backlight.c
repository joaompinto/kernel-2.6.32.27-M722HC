...
static s32 rk29_bl_update_status(struct backlight_device *bl)
{
    ...
    if (ref) {
        ...
        divh = div_total*(BL_STEP-bl->props.brightness)/BL_STEP + BACKLIGHT_REDUCE_VALUE;
		DBG(">>>%s-->%d   div_total == %d , divh == %d\n",__FUNCTION__,__LINE__, div_total, divh);
		divh = divh*MIN_BACKLIGHT_SCALE/10;
		DBG(">>>%s-->%d   bl->props.brightness == %d, div_total == %d , divh == %d\n",__FUNCTION__,__LINE__,bl->props.brightness, div_total, divh);
    }
...
static s32 rk29_bl_get_brightness(struct backlight_device *bl)
{
	...
	divh = divh - BACKLIGHT_REDUCE_VALUE;
	DBG(">>>%s-->%d   div_total == %d , divh == %d\n",__FUNCTION__,__LINE__, div_total, divh);
	divh = divh*10/MIN_BACKLIGHT_SCALE;
	DBG(">>>%s-->%d   div_total == %d , divh == %d\n",__FUNCTION__,__LINE__, div_total, divh);
...
#endif
static void rk29_init_backlight_timer(unsigned long data)
{
...
static void rk29_delaybacklight_timer(unsigned long data)
{
    ...
    if (rk29_bl_info->bl_ref) {
		...
		divh = div_total*(BL_STEP-brightness)/BL_STEP + BACKLIGHT_REDUCE_VALUE;
		DBG(">>>%s-->%d   div_total == %d , divh == %d\n",__FUNCTION__,__LINE__, div_total, divh);
		divh = divh*MIN_BACKLIGHT_SCALE/10;
		DBG(">>>%s-->%d   div_total == %d , divh == %d\n",__FUNCTION__,__LINE__, div_total, divh);
    }
...
{		
    ...
    setup_timer(&rk29_bl_info->init_timer, rk29_init_backlight_timer, (unsigned long)rk29_bl_info);

	rk29_bl_info->init_timer.expires  = jiffies + msecs_to_jiffies(200);
	add_timer(&rk29_bl_info->init_timer);
    ...
    DBG("========%s:: %d div_total: %d, divh = %d\n",__func__,__LINE__,div_total, divh);
